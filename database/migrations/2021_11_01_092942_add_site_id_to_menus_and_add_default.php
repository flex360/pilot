...
class AddSiteIdToMenusAndAddDefault extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        foreach ($tables as $moduleTable) {
            Schema::table($moduleTable, function (Blueprint $table) {
                $table->unsignedInteger('site_id')->nullable()->after('id')->default(1)->change();
            });
        }

        $moduleTable = \Flex360\Pilot\Facades\Menu::getTable();
        Schema::table($moduleTable, function (Blueprint $table) {
