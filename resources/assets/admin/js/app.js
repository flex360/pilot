...
window.addEventListener('load', (event) => {
    ...
    // makes it where you can have multi-select boxes
    if ($('.select2-exclusive').length > 0) {
        $('.select2-exclusive').select2({
            tags: false,
            tokenSeparators: [','],
            width: '100%'
        });
    ...
    if (domain == 'hanks.flextestdrive.com') {
        var key = "ENTER FROALA KEY HERE FOR DEV SITE";
    } else if (domain == 'hanksfurniture.com') {
        var key = "ENTER KEY HERE FOR PRODUCTION SITE";
    } else {
...
function toggleOther(element) {
    var selectorOfItemsToToggle = element.getAttribute('data-toggle-other');
    var itemsToToggle = document.querySelectorAll(selectorOfItemsToToggle);
    var showItem = null;
  
    if (element.hasAttribute('data-show-value')) {
      var showValue = element.getAttribute('data-show-value');
      if (showValue[0] == '[') {
        showValue = JSON.parse(showValue);
        showItem = showValue.indexOf(element.value) > -1;
      } else {
        showItem = element.value == showValue;
      }
    }
  
    if (element.hasAttribute('data-hide-value')) {
      var hideValue = element.getAttribute('data-hide-value');
      if (hideValue[0] == '[') {
        hideValue = JSON.parse(hideValue);
        showItem = hideValue.indexOf(element.value) == -1;
      } else {
        showItem = element.value != hideValue;
      }
    }
  
    itemsToToggle.forEach(function (itemToToggle) {
      itemToToggle.style.display = showItem ? 'block' : 'none';
    });
  }
  
  window.addEventListener('load', function (event) {
    var otherToggles = document.querySelectorAll('select[data-toggle-other]');
  
    otherToggles.forEach(function (otherToggle) {
      toggleOther(otherToggle);
  
      // set change listener
      otherToggle.addEventListener('change', function (event) {
