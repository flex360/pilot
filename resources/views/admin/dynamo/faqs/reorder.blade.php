            ...
            <div class="col-12 col-lg-11 col-xl-10">
                <div class="card sidebar-card">
                    <div class="card-header">
                        Reorder FAQs In This Category
                        <a href="/" target="_blank" id="preview-btn" class="btn btn-info btn-sm float-right"><i class="fa fa-eye"></i> Preview</a>
                        <a href="/pilot/faqcategory" class="btn btn-primary btn-sm float-right" style="margin-right: 10px;">Back to Faq Categories</a>
                    ...
                    <div class="card-body">
                        ...
                        <input type="hidden" id="faqcategoryID" name="faqcategoryID" value="{{ $faqcategory->id }}">
                        ...
                        @else
                            <p>Drag-n-drop sort the FAQs in the order you'd
                            like them to appear inside this FaqCategory.
                            Click the "Preview" button and a new tab will open up to that category.
                            You can re-order the FAQs again, and refresh the Preview tab until you get the order the way you
                            want it.</p>
                                            ...
                                            @foreach ($items as $item)
                                                <tr class="dynamo-index-row" data-id="{{ $item->id }}">
                                                  <td><i class="fas fa-bars fa-2x"></i></td>
                                                    <td>{!! $dynamo->getIndexValue('question', $item) !!}</td>
                                                    <td>{!! $dynamo->getIndexValue('answer', $item) !!}</td>
                                                    <td>
...
@section('scripts')
    <script src="/pilot-assets/legacy/js/faqcategory-faqs.js"></script>
    <script>
    $(document).ready(function(){
        ...
        var previewUrl = '/faqs#cat-' + categoryNumber + '-faq-1';
